...
def view_results(id):
    if session['username'] == 'admin':
        ...
        return render_template('viewresults.html', student_results=student_results)
    else:
        return redirect('/login')
    ...
    if session['username'] == 'admin':
        try:
            if request.method == 'GET':
                ...
                g.db.execute("INSERT INTO results (student_id, quiz_id, result) VALUES (?, ?, ?)", [request.form['student'], request.form['addquiz'], request.form['add_result']])
                g.db.commit()
                return redirect('/dashboard')
        except Exception as e:
            print(e)
